 ...
 * @return            -
 *
 * @Note              - BLOCKÝNG API. Function can will wait until all the bytes are transmitted.
...
void SPI_SendData(SPI_RegDef_t *pSPIx,uint8_t *pTxBuffer, uint32_t Len)
{
	while(Len > 0)
	{
		...
		while(SPI_GetFlagStatus(pSPIx,SPI_TXE_FLAG)  == FLAG_RESET );//polling for the TXE flag to SET (empty)
	...
	while(Len > 0)
		{
			...
			while(SPI_GetFlagStatus(pSPIx,SPI_RXNE_FLAG)  == (uint8_t)FLAG_RESET );//polling for the TXE flag to SET(empty)
...
void  SPI_SSIConfig(SPI_RegDef_t *pSPIx, uint8_t EnOrDi)
{
	if(EnOrDi == ENABLE)
	{
		pSPIx->CR1 |=  (1 << SPI_CR1_SSI);//enable peripheral
	}else
	{
		pSPIx->CR1 &=  ~(1 << SPI_CR1_SSI);//disable peripheral
	}
...
void  SPI_SSOEConfig(SPI_RegDef_t *pSPIx, uint8_t EnOrDi)
{
	if(EnOrDi == ENABLE)
	{
		pSPIx->CR2 |=  (1 << SPI_CR2_SSOE);//enable peripheral
	}else
	{
		pSPIx->CR2 &=  ~(1 << SPI_CR2_SSOE);//disable peripheral
	}
