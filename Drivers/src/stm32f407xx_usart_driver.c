   ...
   //Loop over until "Len" number of bytes are transferred
	for(uint32_t i = 0 ; i < Len; i++)
	{
		...
		if(pUSARTHandle->USART_Config.USART_WordLength == USART_WORDLEN_9BITS)
		{
			...
			{
				//No parity is used in this transfer , so 9bits of user data will be sent and we need 2 bytes so increment 2 pTxBuffer
	...
	//Implement the code to wait till Transfer Completed flag is set in the SR
