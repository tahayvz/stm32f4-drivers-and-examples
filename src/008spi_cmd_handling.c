...
int main(void){
	...
	while(1)
	{
		...
		//do dummy read to clear off the RXNE. 1 byte garbage cleared in RX buffer
		SPI_ReceiveData(SPI2,&dummy_read,1);


		//send dummy bits(1 byte) to fetch the response(acknowledgment) from the slave
		...
		if(SPI_VerifyResponse(ackbyte))//check bits correct or not
		{
			...
			//send always result in reception.that's why we are again reading data register to clear of RXE

			//do dummy read to clear off the RXNE
			SPI_ReceiveData(SPI2, &dummy_read, 1);

			/*insert some delay so that slave can ready with the data. Slave takes some time to read analog value because of
			 *ADC conversion*/
			delay();
